...
class CatalogueController extends Controller
{
    ...
    {
// Trường dạng file khi lấy dữ liệu data phải except(loại trừ) ~ $data->$request->all(); unset($data['cover']);
        $data = $request->except('cover');
        $data['is_active'] ??=0; // $data['is_active'] = $request->input('is_active',0);

//dd($data);
//dd($request->file('cover'));

        if($request->hasFile('cover')){
            $data['cover'] = Storage::put(self::PATH_UPLOAD,$request->file('cover'));  // 2 tham số : 1 : thư mục upload , 2 file get content
    // Vào .env chỉnh : APP_URL=http://xuong-laravel.test ( bỏ / cuối )
